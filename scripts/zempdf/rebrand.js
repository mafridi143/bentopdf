...
function removeElements(content, filePath) {
  ...
  if (CONFIG.removeLicensingLinks) {
    ...
    const desktopLicensingPattern =
      /\s*<a href="\/licensing\.html" class="nav-link"[^>]*>[^<]*<\/a>\s*/g;
    const desktopMatches = modifiedContent.match(desktopLicensingPattern);
    if (desktopMatches) {
      modifiedContent = modifiedContent.replace(
        desktopLicensingPattern,
        '\n          '
      );
    ...
    const mobileLicensingPattern =
      /\s*<a href="\/licensing\.html" class="mobile-nav-link"[^>]*>[^<]*<\/a>\s*/g;
    const mobileMatches = modifiedContent.match(mobileLicensingPattern);
    if (mobileMatches) {
      modifiedContent = modifiedContent.replace(
        mobileLicensingPattern,
        '\n        '
      );
    ...
    const footerLicensingPattern =
      /\s*<li>\s*<a href="\/licensing\.html"[^>]*>[^<]*<\/a>\s*<\/li>\s*/g;
  ...
  if (CONFIG.hideGitHubStars) {
    ...
    const desktopStarsPattern =
      /(\s*)<a href="https:\/\/github\.com\/alam00000\/bentopdf\/"[^>]*>[\s\S]*?<span id="github-stars-desktop">[\s\S]*?<\/a>/g;
    const desktopStarsMatches = modifiedContent.match(desktopStarsPattern);
    if (desktopStarsMatches) {
      modifiedContent = modifiedContent.replace(
        desktopStarsPattern,
        '$1<!-- GitHub stars badge hidden by ZemPDF rebrand -->'
      );
      removedCount += desktopStarsMatches.length;
    }

    // Mobile GitHub stars badge
    const mobileStarsPattern =
      /(\s*)<a href="https:\/\/github\.com\/alam00000\/bentopdf\/"[^>]*>[\s\S]*?<span id="github-stars-mobile">[\s\S]*?<\/a>/g;
...
function addThemeManager(content, filePath) {
  ...
  if (
    !modifiedContent.includes('theme-manager.ts') &&
    !modifiedContent.includes('theme-manager.js')
  ) {
    // Find the first <script type="module"> tag and add theme manager before it
    const scriptPattern =
      /(\s*)(<script type="module" src="[^"]*lucide-init[^"]*"><\/script>)/;
...
function processFile(filePath) {
  try {
    ...
    // Apply brand replacements
    const { content: brandedContent, replacementCount } = applyReplacements(
      content,
      filePath
    );

    // Remove/hide elements (only for HTML)
    const { content: elementsRemoved, removedCount } = removeElements(
      brandedContent,
      filePath
    );

    // Add theme manager script (only for HTML)
    const { content: finalContent, added: themeAdded } = addThemeManager(
      elementsRemoved,
      filePath
    );
    ...
    if (finalContent !== content) {
      ...
      if (themeAdded)
        stats.themeScriptsAdded = (stats.themeScriptsAdded || 0) + 1;

      const changes = [];
      if (replacementCount > 0)
        changes.push(`${replacementCount} replacements`);
      if (removedCount > 0) changes.push(`${removedCount} elements removed`);
      if (themeAdded) changes.push('theme script added');

      console.log(
        `  ✓ Modified: ${path.relative(CONFIG.htmlRootDir, filePath)} (${changes.join(', ')})`
      );
    } else {
      console.log(
        `  - Skipped:  ${path.relative(CONFIG.htmlRootDir, filePath)} (no changes needed)`
      );
    }
  } catch (error) {
    stats.errors.push({ file: filePath, error: error.message });
    console.error(
      `  ✗ Error:    ${path.relative(CONFIG.htmlRootDir, filePath)} - ${error.message}`
    );
  }
}
...
/**
 * Ensure the rebrand script exists in package.json
 * This fixes the issue when upstream merge overwrites package.json
 */
function ensurePackageJsonScript() {
  const packageJsonPath = path.join(CONFIG.htmlRootDir, 'package.json');

  try {
    const content = fs.readFileSync(packageJsonPath, 'utf8');
    const pkg = JSON.parse(content);

    if (!pkg.scripts || !pkg.scripts.rebrand) {
      console.log('⚠ Adding rebrand script to package.json...');
      if (!pkg.scripts) pkg.scripts = {};
      pkg.scripts.rebrand = 'node scripts/zempdf/rebrand.js';
      fs.writeFileSync(
        packageJsonPath,
        JSON.stringify(pkg, null, 2) + '\n',
        'utf8'
      );
      console.log('  ✓ Added rebrand script to package.json\n');
    }
  } catch (error) {
...
function rebrand() {
  ...
  console.log(
    '║  Replacing: BentoPDF → ZemPDF, © 2025 → © 2026                 ║'
  );
  console.log(
    '║  Removing:  Licensing links, GitHub stars badges              ║'
  );
  console.log(
    '║  Preserves: GitHub repo links, npm package names              ║'
  );
  console.log(
    '╚════════════════════════════════════════════════════════════════╝\n'
  );

  // Ensure rebrand script exists in package.json (fixes upstream merge issue)
  ensurePackageJsonScript();

  // Process root HTML files
  console.log('📁 Processing root HTML files...');
  const rootHtmlFiles = getFiles(
    CONFIG.htmlRootDir,
  ...
  console.log(
    `║  Files processed:    ${String(stats.filesProcessed).padEnd(41)}║`
  );
  console.log(
    `║  Files modified:     ${String(stats.filesModified).padEnd(41)}║`
  );
  console.log(
    `║  Total replacements: ${String(stats.replacementsTotal).padEnd(41)}║`
  );
  console.log(
    `║  Elements removed:   ${String(stats.elementsRemoved).padEnd(41)}║`
  );

  if (stats.errors.length > 0) {
    console.log(
      `║  Errors:             ${String(stats.errors.length).padEnd(41)}║`
    );
  }
  ...
  console.log(
    '  3. Commit changes: git add . && git commit -m "Apply ZemPDF branding"'
  );
  console.log('');
}
